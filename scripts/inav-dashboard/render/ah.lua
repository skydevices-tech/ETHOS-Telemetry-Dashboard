...
local function sval(sensors, key, default)
    ...
    v = tonumber(v); if v == nil then return default end
    return v
end
...
local function build_frame(sensors, units, x, y, w, h, opts)
    ...
    -- colors (captured as raw values to avoid lookup in paint)
    local colors = {
    ...
    local ground_sky = {
        base = {x=x, y=y, w=w, h=h},
        baseFillColor = baseFillColor,
        overlayColor  = overlayColor,
        t1 = {rxL + sx, ryL + sy, rxR + sx, ryR + sy, rxR, ryR},
        t2 = {rxL + sx, ryL + sy, rxR,      ryR,      rxL, ryL},
    }

    -- crosshair
    local crosshair = {
        lines  = { {cx-5,cy, cx+5,cy}, {cx,cy-5, cx,cy+5} },
        circle = {cx, cy, 3},
    }
    ...
    if show_compass then
        ...
        for d = -90, 90, 10 do
            ...
            if px > x and px < x+w then
                ...
                if tickHdg % 30 == 0 then
                    texts[#texts+1] = {t=(labels[tickHdg] or string.format("%d", tickHdg)), x=px, y=ribbonY - th - 8, fl=CENTERED + FONT_XS}
                end
            end
        end
        compass = {
            ...
            ticks = ticks,
            texts = texts,
            box = {bw=60,bh=14, bx=cx-30, by=ribbonY+6, txt=string.format("%03d°", hdg)},
            tri = {cx, ribbonY + 1, cx - 5, ribbonY - 7, cx + 5, ribbonY - 7}
        }
    end
    ...
    return {
        ...
        bars=bars,
        show = {
            arc=show_arc, ladder=show_ladder, compass=show_compass,
            altitude=show_altitude, groundspeed=show_groundspeed
        }
    }
end
...
function RenderAH.paint()
    ...
    local t1 = F.ground_sky.t1; lcd.drawFilledTriangle(t1[1],t1[2], t1[3],t1[4], t1[5],t1[6])
    local t2 = F.ground_sky.t2; lcd.drawFilledTriangle(t2[1],t2[2], t2[3],t2[4], t2[5],t2[6])

    -- Crosshair
    lcd.color(F.colors.crosshair)
    local L1 = F.crosshair.lines[1]; lcd.drawLine(L1[1],L1[2], L1[3],L1[4])
    local L2 = F.crosshair.lines[2]; lcd.drawLine(L2[1],L2[2], L2[3],L2[4])
    local C = F.crosshair.circle; lcd.drawCircle(C[1], C[2], C[3])

    -- Arc
    if F.show.arc and F.arc then
        lcd.color(F.colors.arc)
        local ticks = F.arc.ticks
        for i=1,#ticks do local t=ticks[i]; lcd.drawLine(t[1],t[2], t[3],t[4]) end
        local a = F.arc.tri; lcd.drawFilledTriangle(a[1],a[2], a[3],a[4], a[5],a[6])
    end
    ...
    if F.show.ladder and F.ladder then
        ...
        for i=1,#segs do local s=segs[i]; lcd.drawLine(s[1],s[2], s[3],s[4]) end
        local labels = F.ladder.labels
        for i=1,#labels do local lbl=labels[i]; lcd.drawText(lbl.x, lbl.y, lbl.t, lbl.fl) end
    end
    ...
    if F.show.compass and F.compass then
        ...
        for i=1,#ticks do local t=ticks[i]; lcd.drawLine(t[1],t[2], t[3],t[4]) end
        local tri = F.compass.tri; lcd.drawFilledTriangle(tri[1],tri[2], tri[3],tri[4], tri[5],tri[6])
        ...
        lcd.color(F.colors.compass); lcd.drawRectangle(bx, by, bw, bh)
        lcd.drawText(cx, by + 1, F.compass.box.txt, CENTERED + FONT_XS)

        local texts = F.compass.texts
        for i=1,#texts do local t=texts[i]; lcd.drawText(t.x, t.y, t.t, t.fl) end
    end
    ...
    if F.show.altitude and F.bars.alt then
        ...
        local B = F.bars.alt
        local r = B.rect; lcd.drawRectangle(r[1],r[2], r[3],r[4])
        local f = B.fill; lcd.drawFilledRectangle(f[1],f[2], f[3],f[4])
